...
export default {
    ...
    methods: {
       ...
       //返回
       resetForm(){
         this.$tab.closeOpenPage({path:this.$cache.local.getJSON('tg-addback').backurl})
       },
       handleChange(val) {
            console.log(val);
        },
                         ...
                         if (res != undefined) {
                                if (res.code === 200) {
                                   this.$nextTick(function () {
                                     ...
                                     this.$tab.closeOpenPage({ path:this.$cache.local.getJSON('tg-addback').backurl }).then(()=>{
                                        this.$tab.refreshPage({path:this.$cache.local.getJSON('tg-addback').backurl,name:this.$cache.local.getJSON('tg-addback').name});
                                     })
