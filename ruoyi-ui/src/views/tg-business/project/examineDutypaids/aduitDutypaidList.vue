    ...
    methods: {
         ...
         find(row,code){
           ...
           this.$cache.local.setJSON("projectListNews", row);
            let obj={
                backurl:'/tg-business/project/aduitDutypaidList'
            };
            this.$cache.local.setJSON('tg-backTicket',obj);
            this.$tab.closeOpenPage({ path: '/tg-business/project/aduitDutypaidDetail' });
        },
        //完税修改
        edit(row,code){
             let obj = {
             name: 'AduitDutypaidList',
             url: '/tg-business/project/aduitDutypaidList',
            };
