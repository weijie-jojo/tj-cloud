...
import crudInformation from '@/api/tg-api/company/information'
...
export default {
  ...
  mounted() {
    this.getLoginInfo();
    var employedName=this.$cache.local.getJSON('tg-namelist');
  ...
  methods: {
    ...
    isRandom(){
      if(!this.formData.random){
        ...
        this.formData.poposedName1=this.$cache.local.getJSON('tg-namelist').poposedName1;
        this.formData.poposedName2=this.$cache.local.getJSON('tg-namelist').poposedName3;
        this.formData.poposedName3=this.$cache.local.getJSON('tg-namelist').poposedName3;
        this.formData.poposedName4=this.$cache.local.getJSON('tg-namelist').poposedName4;
        this.formData.poposedName5=this.$cache.local.getJSON('tg-namelist').poposedName5;
        this.formData.fontSize1=this.$cache.local.getJSON('tg-namelist').fontSize1;
        this.formData.fontSize2=this.$cache.local.getJSON('tg-namelist').fontSize2;
        this.formData.fontSize3=this.$cache.local.getJSON('tg-namelist').fontSize3;
        this.formData.fontSize4=this.$cache.local.getJSON('tg-namelist').fontSize4;
        this.formData.fontSize5=this.$cache.local.getJSON('tg-namelist').fontSize5;
      }else{
        this.isDisable=true
    ...
    submitForm() {
      this.$refs['elForm'].validate(valid => {
        ...
        if (valid) {
          ...
          if(this.$cache.local.getJSON('tg-namelist').infoStatus==0 || this.$cache.local.getJSON('tg-namelist').infoStatus==1){
            
             let parmsEmployed={
              selfId:this.$cache.local.getJSON('tg-namelist').selfId,
              endStatus:0,
             }
            ...
            this.$tab.closeOpenPage( {path:"/tg-business/reg/employed"} ).then(() => {
                this.$tab.refreshPage( {path:"/tg-business/reg/employed",name:'Employed'} );
               })
             })
