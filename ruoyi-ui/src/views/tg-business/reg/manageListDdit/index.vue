  ...
  methods: {
    ...
    selectAccountType(value) {
      if (value == 1) {
        ...
        this.formData.privateDepositBank = this.$cache.local.getJSON('tg-infolist').privateDepositBank;
        this.formData.privateAccountNumber = this.$cache.local.getJSON('tg-infolist').privateAccountNumber;
    ...
    submitForm() {
       this.$refs['elForm'].validate(valid => {
        ...
        if (valid) {
          ...
          this.$tab.closeOpenPage({ path: this.$cache.local.getJSON('tg-backurls').backUrl }).then(() => {
            this.$tab.refreshPage({ path: this.$cache.local.getJSON('tg-backurls').backUrl, name: this.$cache.local.getJSON('tg-backurls').backName });
          })
        } else {
