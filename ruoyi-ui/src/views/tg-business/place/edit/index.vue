...
export default {
 ...
 methods: {
  editUserid(e){
    ...
    backAgo(){
        this.$tab.closeOpenPage({path:'/tg-business/place/placelist'}).then(() => {
           this.$tab.refreshPage({path:'/tg-business/place/placelist',name:'Placelist'});
    ...
    confirmEdit() {
      if(this.ruleForm.editIsSlider==0){
        if(this.ruleForm.editSpecialSelfFee<=0){
      ...
      if(this.ruleForm.editIsSliderOrdinary==0){
        if(this.ruleForm.editOrdinarySelfFee<=0){
