...
export default {
  ...
  created() {},
  mounted() {
    this.obj = this.$cache.local.getJSON("tj-successProject");
      ...
      if (this.shenmsgs == 1 || this.shenmsgs == 4 || this.shenmsgs==5 ) {
        if (this.$cache.local.getJSON("tj-ifcollect") == 0) {
         ...
         if(this.shenmsgs==5){
          ...
          this.$tab
            .closeOpenPage({
              path: this.$cache.local.getJSON("tj-successProject").backUrl,
            })
            .then(() => {
              this.$tab.refreshPage({
                path: this.$cache.local.getJSON("tj-successProject").backUrl,
                name: this.$cache.local.getJSON("tj-successProject").backName,
              });
            });
        }
      } else  {
