...
import { detailPurch,editPurch } from "@/api/project/list";
...
export default {
  ...
  methods: {
    onSubmit() {
              ...
              if (res.code === 200) {
                this.$nextTick(function () {
                 this.$modal.msgSuccess("出款审核修改完成");
                  this.$tab
                    .closeOpenPage({
                      path: '/tj-business/project/purchList',
                    })
                    .then(() => {
     ...
     getlist() {
      detailPurch(
        this.$cache.local.getJSON("tj-purchId")
      ).then((response) => {
       this.formData=response.data;
      });
    },
    resetForm() {
      this.$tab.closeOpenPage({ path: "/tj-business/project/purchList" });
    },
