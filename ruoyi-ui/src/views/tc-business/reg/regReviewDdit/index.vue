      ...
      <el-row type="flex" class="row-bg rowCss" justify="space-around">
        <el-col :span="9">
          <el-form-item class="comright" label="民族" prop="nation">
            <el-input v-model="formData.nation" clearable> </el-input>
          </el-form-item>
        </el-col>
        <el-col :span="9">
        ...
        >
          <el-col :span="9">
            <el-form-item
              ...
              :required="true"
            >
              <el-radio v-model="formData.isPublicUser" label="0"
                >开启</el-radio
              >
              <el-radio v-model="formData.isPublicUser" label="1"
                >关闭</el-radio
              >
            </el-form-item>
          </el-col>
...
export default {
  ...
  methods: {
    ...
    submitForm() {
     this.$refs["elForm"].validate((valid) => {
        if (valid) {
          ...
          this.$tab
            .closeOpenPage({
              path: this.$cache.local.getJSON("tc-backurls").backUrl,
            })
            .then(() => {
              this.$tab.refreshPage({
                path: this.$cache.local.getJSON("tc-backurls").backUrl,
                name: this.$cache.local.getJSON("tc-backurls").backName,
              });
            });
        } else {
    ...
    resetForm() {
      this.$tab
        .closeOpenPage({ path: this.$cache.local.getJSON("tc-backurls").backUrl })
        .then(() => {
          this.$tab.refreshPage({
            path: this.$cache.local.getJSON("tc-backurls").backUrl,
            name: this.$cache.local.getJSON("tc-backurls").backName,
      ...
      all()
        .then((res) => {
          if (res != undefined) {
            ...
            for (let i in res) {
              this.accountName_options.push({
                value: res[i].publicDepositBank3,
              });
            }
          }
        })
