...
import { joinList, getCount, delEmployed, checkdetail } from "@/api/tc-api/company/employed";
...
export default {
  ...
  methods: {
    ...
    examine(applyName, scope, type) {
      let obj = {
        backUrl: '/tc-business/reg/employed',
      };
      ...
      getUser(applyName).then(res => {
        ...
        this.$confirm(
          ...
          if (msgs == '查看') {
            ...
            switch (type) {
              case 1:
                this.$cache.local.setJSON('tc-namelist', scope.selfCode);
                this.$tab.refreshPage({path:"/tc-business/reg/nameDetail",name:'NameDetail'})
                break;
              case 2:
                this.$cache.local.setJSON('tc-infolist', scope.selfCode);
                this.$tab.refreshPage({path:"/tc-business/reg/infoDetail",name:'InfoDetail'})
                 break;
              case 4:
                 this.$cache.local.setJSON('tc-businesslist', scope.selfCode);
                 this.$tab.refreshPage({path:"/tc-business/reg/detailBusiness",name:'DetailBusiness'})
              break;
              case 5:
                this.$cache.local.setJSON('tc-taxlist', scope.selfCode);
                this.$tab.refreshPage({path:"/tc-business/reg/detailTax",name:'DetailTax'});
                 break;
              case 6:
                this.$cache.local.setJSON('tc-banklist', scope.selfCode);
                this.$tab.refreshPage({path:"/tc-business/reg/detailBank",name:'DetailBank'});
                break;
              case 7:
                this.$cache.local.setJSON("tc-confirmlist", scope.selfCode);
                this.$tab.refreshPage({path:"/tc-business/reg/regReviewDetail",name:'RegReviewDetail'});
                break;
            ...
            switch (type) {
              case 1:
              this.$cache.local.setJSON('tc-namelist', scope.selfCode);
              this.$tab.refreshPage({path:"/tc-business/reg/editEmployedName",name:'EditEmployedName'})
              break;
              case 2:
              this.$cache.local.setJSON('tc-infolist', scope.selfCode);
              this.$tab.refreshPage({path:"/tc-business/reg/editEmployedInfo",name:'EditEmployedInfo'})
               break;
            ...
            switch (type) {
              case 1:
                ...
                this.$cache.local.setJSON('tc-namelist', scope.selfCode);
                this.$tab.refreshPage({path:"/tc-business/reg/nameAduit",name:'NameAduit'});
              ...
              case 2:
                ...
                this.$cache.local.setJSON('tc-resultBack', obj1);
                this.$cache.local.setJSON('tc-infolist', scope.selfCode);
                this.$tab.refreshPage({path:"/tc-business/reg/infoAduit",name:'InfoAduit'});
                ...
                this.$cache.local.setJSON('tc-resultBack', obj2);
                this.$cache.local.setJSON('tc-businesslist', scope.selfCode);
                ...
                this.$cache.local.setJSON('tc-resultBack', obj3);
                this.$cache.local.setJSON('tc-taxlist', scope.selfCode);
                ...
                this.$cache.local.setJSON('tc-resultBack', obj4);
                this.$cache.local.setJSON('tc-banklist', scope.selfCode);
                ...
                this.$cache.local.setJSON('tc-resultBack', obj5);
                this.$cache.local.setJSON('tc-confirmlist', scope.selfCode);
        ...
        checkdetail(parms).then(err => {
          ...
          this.$confirm(
            ...
            if (this.errConfirmsMsg == '修改') {
              this.$tab.refreshPage({path:"/tc-business/reg/regReviewDdit",name:'RegReviewDdit'});
              } else {
              this.$tab.refreshPage({path:"/tc-business/reg/regReviewDetail",name:'RegReviewDetail'});
             }
            }).catch(() => {});
   ...
   check(self_code, self_type) {
    ...
    /** 新增按钮操作 */
    handleAdd() {
      this.$tab.refreshPage({path:"/tc-business/reg/addEmployedInfo",name:'AddEmployedInfo'});
    },
    /** 删除按钮操作 */
    handleDelete() {
