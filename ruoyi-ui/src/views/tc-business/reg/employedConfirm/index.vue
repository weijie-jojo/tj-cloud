<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="employedList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="客户全名" width="400" align="center" prop="placeAliasName" :show-overflow-tooltip="true"/>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="text" v-hasPermi="['company:employedConfirm:edit']" v-if="scope.row.endStatus == 2" icon="el-icon-edit"
            @click="edits(scope.row)">修改</el-button>
          <el-button v-hasPermi="['company:employedConfirm:edit']"  size="mini" type="text" icon="el-icon-edit" v-else
            style="color: #C0C4CC!important;border:0 !important;background-color:rgba(0,0,0,0) !important" plain disabled>修改</el-button>
          <el-button v-hasPermi="['company:employedConfirm:confirm']" size="mini" v-if="scope.row.endStatus == 0" type="text" icon="el-icon-s-check"
            @click="addConfirm(scope.row)">注册确认办理</el-button>
          <el-button  v-hasPermi="['company:employedConfirm:confirm']" size="mini" v-else icon="el-icon-s-check"
...
import { joinList,getCount} from "@/api/tc-api/company/employed";
...
export default {
  ...
  methods: {
    ...
    edits(row) {
       let obj = {
        name:'注册确认',
        backUrl: '/tc-business/reg/employedConfirm',
        backName: "EmployedConfirm",
      };
      this.$cache.local.setJSON('tc-backurls', obj);
      this.$cache.local.setJSON("tc-confirmlist", row.selfCode);
