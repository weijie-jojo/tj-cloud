      ...
      <el-table-column type="selection" width="55" align="center" />
       ...
       <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button size="mini" v-hasPermi="['company:bank:examine']" v-if="scope.row.bankStatus == 0" type="text" icon="el-icon-s-check"
            @click="bank(scope.row)">银行办理</el-button>
          <el-button size="mini" v-hasPermi="['company:bank:examine']" v-else icon="el-icon-s-check" style="color: #C0C4CC!important;border:0 !important;background-color:rgba(0,0,0,0) !important" plain disabled>银行办理</el-button>
      </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
     ...
     if(this.bankStatus=='-1'){
      ...
      detail(row){
        let obj = {
          backUrl: '/tc-business/reg/manageBank',
         };
         this.$cache.local.setJSON('tc-backurls', obj);
         this.$cache.local.setJSON('tc-banklist', row.selfCode);
