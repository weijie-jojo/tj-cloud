...
import { list, list2, delReal } from "@/api/tc-api/project/ticket";
...
export default {
  ...
  methods: {
    ...
    detail(row) {
      this.$cache.local.setJSON("tc-ticketid", row.ticketId);
      this.$tab.openPage("票据查看", "/tc-business/project/ticketDetail");
    },
    ...
    /** 新增按钮操作 */
    handleAdd() {
      this.$tab.closeOpenPage({ path: "/tc-business/project/ticketAdd" });
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.$cache.local.setJSON("tc-ticketid", row.ticketId);
      this.$tab.closeOpenPage({ path: "/tc-business/project/ticketEdit" });
    },
