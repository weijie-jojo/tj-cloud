...
export default {
    ...
    methods: {
        ...
        //返回
        resetForm() {
            this.$tab.closeOpenPage({ path: this.$cache.local.getJSON('tc-edit-project').url })
        },
        handleChange(val) {
            console.log(val);
        },
                        ...
                        if (res != undefined) {
                            if (res.code === 200) {
                                this.$nextTick(function () {
                                    ...
                                    this.$tab.closeOpenPage({ path: this.$cache.local.getJSON('tc-edit-project').url }).then(() => {
                                        // 执行结束的逻辑 
                                        this.$tab.refreshPage({ path: this.$cache.local.getJSON('tc-edit-project').url, name: this.$cache.local.getJSON('tc-edit-project').name })
                                    })
