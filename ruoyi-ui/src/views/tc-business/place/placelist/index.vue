...
export default {
  ...
  methods: {
    ...
    /** 新增按钮操作 */
    handleAdd() {
      this.$tab.refreshPage({path:'/tc-business/place/add',name:'Add'});
     
    },
    /** 修改按钮操作 */
    handleUpdate(item) {
      var placeCode = item.placeCode;
      this.$cache.local.setJSON("tc-placeItem",item);
      this.$modal.loading("正在加载数据，请稍后...");
      agencyfee.selectFeeByCode({ placeCode: placeCode }).then(res => {
      this.$modal.closeLoading();
      this.$cache.local.setJSON("tc-placeItems",res);
      this.$tab.refreshPage({path:'/tc-business/place/edit',name:'Edit'});
    ...
    detail(item) {
      ...
      this.$cache.local.setJSON("tc-placeDetailItem",item);
      this.$modal.loading("正在加载数据，请稍后...");
      agencyfee.selectFeeByCode({ placeCode: item.placeCode }).then(res => {
        this.$modal.closeLoading();
        this.$cache.local.setJSON("tc-placeDetailItem",res);
