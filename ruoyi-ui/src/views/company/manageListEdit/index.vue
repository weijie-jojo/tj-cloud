  ...
  methods: {
    onSubmit(){
        ...
        if (valid) {
          ...
          updateEmployed(this.formData)
            .then((res) => {
              if (res != undefined) {
                if (res.code === 200) {
                  this.$modal.msgSuccess("修改成功");
                  this.$nextTick(function () {
                    this.$tab.refreshPage("/customer/manageList").then(() => {
                      this.$tab.openPage("个体户注册", "/customer/manageList");
                    });
    ...
    resetForm(){
      this.$tab.openPage("个体户注册", "manageList").then(() => {
       // 执行结束的逻辑
      })
    },
    handleChange(val) {
