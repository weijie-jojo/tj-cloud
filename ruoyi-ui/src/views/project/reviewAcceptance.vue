        ...
        <el-table v-loading="loading" :data="projectList" @selection-change="handleSelectionChange">
            ...
            <el-table-column label="完结状态" align="center" prop="projectStatus">
                <template slot-scope="scope">
                    <el-link :underline="false" type="danger" v-if="scope.row.projectAcceptanceStatus == '1'">异常</el-link>
                    <el-link :underline="false" type="success" v-if="scope.row.projectAcceptanceStatus == '2'">完成</el-link>
                    <el-link :underline="false" type="primary" v-if="scope.row.projectAcceptanceStatus == '0'">办理中</el-link>
                </template>
            </el-table-column> 
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
                <template slot-scope="scope">
                    <el-button size="mini" v-if="!scope.row.fileName2" type="text" icon="el-icon-add"
                        @click="details(scope.row)">验收办理</el-button>
                    <el-button size="mini" v-else type="text" icon="el-icon-s-custom" @click="detail(scope.row)">审核验收
                    </el-button>
                      <el-button size="mini" v-if="scope.row.projectAcceptanceStatus==2" type="text" icon="el-icon-s-custom" @click="find(scope.row,scope.row.projectCode)">查看验收</el-button>
                    <el-button size="mini" v-if="scope.row.projectAcceptanceStatus==1" type="text" icon="el-icon-s-custom" @click="edits(scope.row,scope.row.projectCode)">编辑验收
                    </el-button>
                </template>
            </el-table-column>
        </el-table>
