 ...
 */
@RestController
@RequestMapping("/receive/tc")
@Api(tags = "收款信息表（腾创）")
public class SelfReceiveTcController extends BaseController
{
    @Autowired
    private ISelfReceiveTcService selfReceiveService;
    ...
    public String getCode(){
        ...
        String code="";
        if (selfReceive !=null){
            code=  StringUtils.getCode("TCRECEIVE", selfReceive.getReceiveSysCode(),"yyyyMMdd");
        }else {//没有数据时
            code="TCRECEIVE"+"-"+nowDate+"-"+"0001";
        }
        return code;
    };
