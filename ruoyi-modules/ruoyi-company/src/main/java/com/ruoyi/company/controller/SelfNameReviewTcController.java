...
{
    @Autowired
    private ISelfNameReviewTcService selfNameReviewService;
    @Autowired
...
//    @RequiresPermissions("company:review:list")
    ...
    public String getSelfCode() {
        ...
        if (selfNameReviews.size()>0){
            selfCode=  StringUtils.getCode("TCYWGTH",employeeNumber,selfNameReviews.get(0).getSelfCode());
        }else {//没有单据时
            selfCode="TCYWGTH"+employeeNumber+"00001";
        }
        System.out.println("selfCode==="+selfCode);
        return  selfCode;
    }
