...
import com.ruoyi.company.service.ISelfNameReviewTgService;
 ...
 */
@RestController
@RequestMapping("/review/tg")
@Api(tags = "个体户名字审核（腾工）")
public class SelfNameReviewTgController extends BaseController
{
    @Autowired
    private ISelfNameReviewTgService selfNameReviewService;
    @Autowired
...
//    @RequiresPermissions("company:review:list")
    ...
    public String getSelfCode() {
        ...
        if (selfNameReviews.size()>0){
            selfCode=  StringUtils.getCode("TGYWGTH",employeeNumber,selfNameReviews.get(0).getSelfCode());
        }else {//没有单据时
            selfCode="TGYWGTH"+employeeNumber+"00001";
        }
        System.out.println("selfCode==="+selfCode);
        return  selfCode;
    }
